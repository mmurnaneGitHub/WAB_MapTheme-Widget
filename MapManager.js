  ...
  var instance = null,
    clazz = declare(null, {
      ...
      _addDataLoadingOnMapUpdate: function(map) {
        //MJM - Reset the Default extent - otherwise it becomes the current map extent (this.appConfig.map.mapOptions.extent) - SEE THEME WIDGET.JS FOR REST OF FUNCTIONALITY
        if (typeof this.appConfig.map.mapOptions.lastExtent !== 'undefined') {  //Not the intial load of widget, web map has been switched, and variable exists (created by Theme widget).
          this.map.setExtent(this.appConfig.map.mapOptions.lastExtent);  //Zoom from the current web map default to map extent used before the web map switch 
        }
        //end map extent reset
